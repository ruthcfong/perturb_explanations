% copied from ILSVRC2014_devkit
function ov_vector = compute_overlap(bb,gt_bbox,gt_label)
    ov_vector = [];
    for j=1:numel(gt_bbox.objects)
        if isempty(gt_bbox.objects(j).label)
            warning('skipping object because it does not have a label.'); % happens for annotated 61 (window)
            continue;
        end
