...
[heatmap, res] = compute_heatmap(net, img, target_class, 'excitation_backprop', norm_deg, opts);

%[heatmap_lrp2, res_lrp2] = compute_heatmap(net, img, target_class, 'lrp_alpha_beta', norm_deg, opts);
%[heatmap_eb2, res_eb2] = compute_heatmap(net, img, target_class, 'excitation_backprop', norm_deg, opts);
...
for i=1:length(layer_idx)
    ...
    % layer_i = find(cellfun(@(x) strcmp(x, layer_names{i}), {net.layers.name})); % for dag
